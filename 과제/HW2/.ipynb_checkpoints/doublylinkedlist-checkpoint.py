...
def constructDLLRecursiveStep(arr, y, x, curr):
    ...
    if(new_node.up == None):
        ...
        if(new_node.up.right != None):
            new_node.right = new_node.up.right.down
            new_node.up.right.down.left = new_node
        else:
            new_node.right = None

    new_node.down = constructDLLRecursiveStep(arr,y+1, x, new_node)
    if (new_node.down != None):
...
def removeNodeRecursive(em_ptr, im_ptr):
    ...
    if em_ptr.right !=None:
        ...
        if np.isneginf(em_ptr.right.up.data):
            ...
            '''if em_ptr.up.left.data == "-inf":
                im_ptr.down.left = im_ptr.left
            elif em_ptr.down.left.data == "-inf":
                im_ptr.down.left = im_ptr.left'''
            return 
