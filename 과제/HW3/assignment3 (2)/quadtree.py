...
def build_quadtree_recursive(qt, node):
    ...
    """
    if node.dr_x == node.ul_x and node.dr_y == node.ul_y:
        node.color = qt.image[node.ul_y][node.ul_x]

    else:
        node.ne = build_quadtree_recursive(qt, Node(None, compute_image_corners(node, 'ne'),node.level+1))
        node.nw = build_quadtree_recursive(qt, Node(None, compute_image_corners(node, "nw"),node.level+1))
        node.se = build_quadtree_recursive(qt, Node(None, compute_image_corners(node, "se"),node.level+1))
        node.sw = build_quadtree_recursive(qt, Node(None, compute_image_corners(node, "sw"),node.level+1))
    return node
...
def count_nodes(node):
    ...
    """
    if node.se != None and node.sw != None and node.nw != None and node.ne != None:
        return 1 + count_nodes(node.se) + count_nodes(node.sw) + count_nodes(node.nw) + count_nodes(node.ne)
    else:
        return 1
